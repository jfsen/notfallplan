...
function calculateDays(animate = false) {
    ...
    const berlinNow = new Date(
        now.getTime() + getBerlinOffset(now),
    );
    start.setHours(0, 0, 0, 0);
    berlinNow.setHours(0, 0, 0, 0);
    const diffDays = Math.floor(
        (berlinNow - start) / (1000 * 60 * 60 * 24),
    );
...
function exportCleanHTML() {
    ...
    const viewerScript =
        document.getElementById("viewer-script").textContent;
    ...
    tempContainer
        ...
        .forEach((item) => {
            ...
            item.querySelectorAll("[style*='cursor']").forEach(
                (el) => el.style.removeProperty("cursor"),
            );
        });
